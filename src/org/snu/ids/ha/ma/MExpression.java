...
public class MExpression extends ArrayList<MCandidate>
    ...
    void prune() {
        ...
        int i = pruneIdx;
        for (int stop = size(); i < stop; i++) {
            if ((uncomplete) && (i == stop - 1) && (((MCandidate) get(pruneIdx)).realDicLen == 0) && (((MCandidate) get(pruneIdx)).getExp().length() < 10)) {
                break;
            }
            remove(pruneIdx);
        }
        ...
        for (int i = 0; i < thisMESize; i++) {
            ...
            for (int j = 0; j < preMESize; j++) {
                ...
                if ((preMC.isApndblWithSpace(mcThis)) || (preMC.isApndbl(mcThis))) {
                    MCandidate tmp82_80 = mcThis;
                    tmp82_80.numOfApndblMC = (byte) (tmp82_80.numOfApndblMC + 1);
                    break;
                }
            }
        ...
        for (int i = 0; i < thisMESize; i++) {
            ...
            for (int j = 0; j < nextMESize; j++) {
                ...
                if (thisMC.isApndblWithSpace(nextMC)) {
                    MCandidate tmp67_65 = thisMC;
                    tmp67_65.numOfApndblMC = (byte) (tmp67_65.numOfApndblMC + 1);
                    break;
                }
            }
    ...
    MExpression[] divideHeadTailAt(String headStr, int headIndex, String tailStr, int tailIndex)
        ...
        MExpression headME;
        headME = ret[0] = new MExpression(headStr);
        MExpression tailME;
        tailME = ret[1] = new MExpression(tailStr);

        int j = 0;
        for (int stop = size(); j < stop; j++) {
    ...
    void sortByLnpr() {
        Collections.sort(this, new Comparator() {
            ...
            public int compare(Object obj, Object obj1) {
                return compare((MCandidate) obj, (MCandidate) obj1);
            }
        });
    }
    ...
    void sortByBestLnpr() {
        Collections.sort(this, new Comparator() {
            ...
            public int compare(Object t, Object t1) {
               return compare((MCandidate)t, (MCandidate)t1);
             }
        });
    }
    ...
    boolean isOneEojeolCheckable() {
        if (size() == 1) {
            ...
            if ((mc.size() == 1) && ((mp.isCharSetOf(CharSetType.NUMBER))
