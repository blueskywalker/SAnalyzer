...
public class Dictionary {
    ...
    public boolean containNoun(String exp) {
        ...
        for (Iterator iterator = me.iterator(); iterator.hasNext();) {
            MCandidate mc = (MCandidate) iterator.next();
            if (mc.size() == 1 && mc.isFirstTagOf(POSTag.NNA)) {
                return true;
            }
        }
    ...
    public List<Morpheme> getWordList() {
        ...
        for (Iterator iterator = table.values().iterator(); iterator.hasNext();) {
            MExpression me = (MExpression) iterator.next();
            for (Iterator iterator1 = me.iterator(); iterator1.hasNext();) {
                MCandidate mc = (MCandidate) iterator1.next();
                if (mc.size() == 1) {
